...
from .chunk_util import fwd_decay_cumsum, prepare_qg_kg
...
def fused_chunk_gla_fwd_kernel(
        ...
        T,  # seq_len
        scale,  # D_head_K ** -0.5
        BT: tl.constexpr,  # BLOCK SIZE along the sequence dimension
    ...
    p_q = tl.make_block_ptr(
        q + i_bh * s_qk_h,  # base
        (T, DK),  # shape of the parent tensor
        (s_qk_t, s_qk_d),  # strides of the parent tensor
        (0, i_k * BK),  # offset to block
        (BT, BK),  # block shape
        (1, 0))  # order
    ...
    if STORE_FINAL_STATE:
        p_final = tl.make_block_ptr(
            ...
            (1, 0))
        tl.store(p_final,
                 b_h.to(p_final.dtype.element_ty),
                 boundary_check=(0, 1))
...
def fwd_inner_chunk(
    ...
    p_A = A + (i_bh + (i_k * B * H)) * (tl.cdiv(T, BT) * BT *
                                        BT) + i_t * BT * BT + tl.arange(0, BT)
...
class FusedChunkGLAFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, g, scale, initial_state, output_final_state):
        ...
        # cumulative decay should be in float32,
        # otherwise the err will be accumulated and amplified.
        ...
        fwd_decay_cumsum[grid](g_original,
                               g,
                               q.stride(1),
                               q.stride(2),
                               q.stride(3),
        ...
        if output_final_state:
            final_state = q.new_empty(batch_size,
                                      n_heads,
                                      d_head_qk,
                                      d_head_v,
                                      dtype=torch.float,
        ...
        if version.parse(triton.__version__) < version.parse('2.2.0'):
            ...
            warnings.warn(
                ...
                "which is known to have some weird compiler issues"
                "(refer to https://github.com/openai/triton/issues/2852) "
        ...
        fused_chunk_gla_fwd_kernel[grid](q_g,
                                         k_g,
                                         v,
                                         g,
                                         o,
        ...
        A = q.new_empty(NK, batch_size, n_heads, triton.cdiv(seq_len, BT), BT,
                        BT)

        grid = (NK, triton.cdiv(seq_len, BT), batch_size * n_heads)
        fwd_inner_chunk[grid](q,
                              k,
                              g,
                              A,
                              q.stride(1),
...
def fused_chunk_gla(
    ...
    o, final_state = FusedChunkGLAFunction.apply(q, k, v, g, scale,
                                                 initial_state,
                                                 output_final_state)
